...
#include <assert.h>
#include "aecm_lib/echo_control_mobile.h"
 ...
 *
 */
JNIEXPORT jlong JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeCreateAecmInstance(JNIEnv *env, jclass thiz) {
 ...
 *
 */
JNIEXPORT jint JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeFreeAecmInstance(JNIEnv *env, jclass thiz, jlong aecmHandler) {
 ...
 *
 */
JNIEXPORT jint JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeInitializeAecmInstance(JNIEnv *env, jclass thiz, jlong aecmHandler, jint sampFreq) {
	void *aecmInst = (void *) aecmHandler;
	if (aecmInst == NULL)
		return -1;
 ...
 *
 */
JNIEXPORT jint JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeBufferFarend(JNIEnv *env, jclass thiz, jlong aecmHandler, jshortArray farend, jint nrOfSamples) {
...
JNIEXPORT jshortArray JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeAecmProcess(JNIEnv *env, jclass thiz, jlong aecmHandler, const jshortArray nearendNoisy, const jshortArray nearendClean, jshort nrOfSamples, jshort msInSndCardBuf) {
	...
	//release arrays back to java side (must always release, even on error)
	(*env)->ReleaseShortArrayElements(env, nearendNoisy, arrNearendNoisy, JNI_ABORT);
	(*env)->ReleaseShortArrayElements(env, out, arrOut, 0);
	if (nearendClean != NULL && arrNearendClean != NULL)
		(*env)->ReleaseShortArrayElements(env, nearendClean, arrNearendClean, JNI_ABORT);
 ...
 *
 */
JNIEXPORT jint JNICALL Java_com_repository_glasses_listener_audio_WebRtcAecm_nativeSetConfig(JNIEnv *env, jclass thiz, jlong aecmHandler, jobject aecmConfig) {
